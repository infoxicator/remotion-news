...
import { P as ProviderError, j as bufferExports, C as CredentialsProviderError, k as parseUrl, m as loadConfig } from "./server-build-DHo3NJYJ.js";
...
function httpRequest(options) {
  return new Promise((resolve, reject) => {
    ...
    req.on("response", (res) => {
      ...
      res.on("end", () => {
        resolve(bufferExports.Buffer.concat(chunks));
        req.destroy();
      });
    });
    req.end();
...
const requestFromEcsImds = async (timeout, options) => {
  if (define_process_env_default[ENV_CMDS_AUTH_TOKEN]) {
    options.headers = {
      ...options.headers,
      Authorization: define_process_env_default[ENV_CMDS_AUTH_TOKEN]
    };
  }
...
const getCmdsUri = async ({ logger }) => {
  if (define_process_env_default[ENV_CMDS_RELATIVE_URI]) {
    return {
      hostname: CMDS_IP,
      path: define_process_env_default[ENV_CMDS_RELATIVE_URI]
    };
  }
  if (define_process_env_default[ENV_CMDS_FULL_URI]) {
    const parsed = parse(define_process_env_default[ENV_CMDS_FULL_URI]);
